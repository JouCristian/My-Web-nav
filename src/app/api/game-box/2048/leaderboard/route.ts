...
import { isCompetitive2048Mode, normalizeBoard2048Size } from "@/features/game-box/2048/lib/modes"
...
export async function GET(request: NextRequest) {
  ...
  const boardSize = normalizeBoard2048Size(request.nextUrl.searchParams.get("boardSize"))
  const session = await auth().catch(() => null)

  try {
    return NextResponse.json(await get2048Leaderboard(period, mode, boardSize, session?.user?.id))
  } catch (error) {
