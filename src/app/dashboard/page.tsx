...
const THEME_MAP = {
  blue: { border: "border-blue-500/20 hover:border-blue-500/60", shadow: "shadow-[0_0_40px_rgba(59,130,246,0.1)] hover:shadow-[0_0_80px_rgba(59,130,246,0.2)]", blob: "bg-blue-500/10 group-hover:bg-blue-500/25", iconBox: "bg-blue-500/10 border-blue-500/20 shadow-[0_0_20px_rgba(59,130,246,0.15)] group-hover:bg-blue-500/20 text-blue-400", subtitle: "text-blue-200/40", activeText: "text-blue-400" },
  purple: { border: "border-purple-500/20 hover:border-purple-500/60", shadow: "shadow-[0_0_40px_rgba(168,85,247,0.1)] hover:shadow-[0_0_80px_rgba(168,85,247,0.2)]", blob: "bg-purple-500/10 group-hover:bg-purple-500/25", iconBox: "bg-purple-500/10 border-purple-500/20 shadow-[0_0_20px_rgba(168,85,247,0.15)] group-hover:bg-purple-500/20 text-purple-400", subtitle: "text-purple-200/40", activeText: "text-purple-400" },
  yellow: { border: "border-amber-500/20 hover:border-amber-500/60", shadow: "shadow-[0_0_40px_rgba(245,158,11,0.1)] hover:shadow-[0_0_80px_rgba(245,158,11,0.2)]", blob: "bg-amber-500/10 group-hover:bg-amber-500/25", iconBox: "bg-amber-500/10 border-amber-500/20 shadow-[0_0_20px_rgba(245,158,11,0.15)] group-hover:bg-amber-500/20 text-amber-400", subtitle: "text-amber-200/40", activeText: "text-amber-400" }
}

const ModuleCard = ({ moduleId, title, subtitle, icon, link, isActive, theme = "purple" }: { moduleId: string; title: string; subtitle: string; icon: string; link: string; isActive: boolean; theme?: "blue" | "purple" | "yellow" }) => {
  const styles = THEME_MAP[theme];
  return (
    <Link href={isActive ? link : "#"} className={`group relative w-full h-[300px] sm:h-[340px] lg:h-[360px] rounded-[2rem] sm:rounded-[3rem] lg:rounded-[3.5rem] border ${styles.border} bg-[#06060a]/95 p-6 sm:p-8 lg:p-12 flex flex-col justify-between overflow-hidden transition-all duration-700 ease-[cubic-bezier(0.34,1.56,0.64,1)] active:scale-[0.98] ${isActive ? `hover:-translate-y-2 ${styles.shadow}` : 'opacity-60 grayscale'}`}>
...
export default async function DashboardPage() {
  ...
  // ==========================================
  // 🛡️ 状态 1 & 2：合并处理新兵引导与审核中逻辑
  // ==========================================
  if (!isCaptain && (isProfileIncomplete || dbUser.role === "PENDING")) {
    // 判定当前到底处于哪个阶段
    const isPendingApproval = !isProfileIncomplete && dbUser.role === "PENDING";
    
    // 动态调整背景光晕和按钮颜色，填表时是警告红，审核中是安全蓝
    const glowColor = isPendingApproval ? "bg-blue-500/10" : "bg-red-500/5";
    const btnDotColor = isPendingApproval ? "bg-blue-400 shadow-[0_0_15px_rgba(96,165,250,0.8)]" : "bg-red-500 shadow-[0_0_15px_rgba(239,68,68,0.8)]";
    const btnRingColor = isPendingApproval ? "border-blue-500/30" : "border-red-500/30";
    const btnHoverBg = isPendingApproval ? "group-hover:bg-blue-500/20" : "group-hover:bg-red-500/20";
    const btnHoverText = isPendingApproval ? "group-hover:text-blue-400" : "group-hover:text-red-400";
    ...
    return (
      <main className="min-h-screen bg-transparent flex flex-col items-center justify-center p-6 relative overflow-hidden">
        ...
        <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[400px] ${glowColor} -rotate-12 blur-[100px] pointer-events-none animate-pulse transition-colors duration-1000`}></div>
        
        {/* 🚀 已经移至最左上角的通用返回按钮 */}
        <div className="absolute top-6 left-4 sm:top-10 sm:left-10 z-50">
          <TransitionLink href="/" className="group flex items-center gap-3 sm:gap-4 bg-black/40 px-4 sm:px-5 py-3 rounded-2xl border border-white/10 backdrop-blur-md transition-all active:scale-95 shadow-[0_0_30px_rgba(0,0,0,0.5)]">
            <div className={`relative flex items-center justify-center w-7 h-7 rounded-full bg-white/5 border border-white/20 transition-colors ${btnHoverBg}`}>
              <div className={`w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full animate-pulse transition-colors ${btnDotColor}`} />
              <div className={`absolute inset-0 rounded-full border animate-[ping_2.5s_cubic-bezier(0,0,0.2,1)_infinite] transition-colors ${btnRingColor}`} />
            </div>
            <div className="flex flex-col items-start">
              <span className={`text-[9px] sm:text-[10px] text-zinc-500 uppercase tracking-[0.2em] font-mono transition-colors ${btnHoverText}`}>Abort Sequence</span>
              <span className="text-sm font-bold text-white tracking-widest font-[family-name:var(--font-space)]">撤离拦截区</span>
            </div>
          </TransitionLink>
        </div>

        <div className="relative z-10 w-full max-w-xl animate-apple-spring opacity-0">
           {/* 如果是审核中，组件内会利用 initialStep=6 强制渲染为完成状态 */}
           <OnboardingForm defaultCompleted={isPendingApproval} />
        </div>
      </main>
    )
  }
