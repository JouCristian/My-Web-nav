...
import { CrewActionButtons } from "@/components/crew-action-buttons" // 🚀 引入审核按钮
      ...
      <div className="max-w-5xl mx-auto relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 border-b border-white/10 pb-6 gap-6">
          ...
          <div className="flex flex-wrap gap-4">
            {isManager && (
              <button className="group flex items-center gap-3 bg-black/40 px-5 py-3 rounded-2xl border border-white/10 backdrop-blur-md animate-flame-hover hover:border-emerald-500/30 transition-all active:scale-95">
                ...
                <div className="flex flex-col items-start text-left">
                  ...
                  <span className="text-sm font-bold text-white tracking-widest">强行建档</span>
                </div>
              </button>
            )}
            <Link href="/dashboard" className="group flex items-center gap-3 bg-black/40 px-5 py-3 rounded-2xl border border-white/10 backdrop-blur-md animate-flame-hover hover:border-purple-500/30 transition-all active:scale-95">
              ...
              <div className="flex flex-col items-start text-left">
                ...
                <span className="text-sm font-bold text-white tracking-widest">返回中枢</span>
              </div>
            </Link>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {sortedUsers.map((user) => {
            ...
            const roleBorder = isOwner ? "border-yellow-500/40 bg-yellow-500/5" : isAdmin ? "border-purple-500/40 bg-purple-500/5" : "border-white/5 bg-black/40";
            ...
            return (
              <div key={user.id} className={`group relative flex flex-col md:flex-row items-start md:items-center justify-between p-5 rounded-2xl border backdrop-blur-md transition-all hover:border-white/20 ${roleBorder}`}>
                <div className="flex items-center gap-5 w-full md:w-auto">
                  ...
                  <div className="flex flex-col">
                    <div className="flex items-baseline gap-3">
                      <span className={`text-lg font-bold tracking-wider ${isOwner ? 'text-yellow-400' : isAdmin ? 'text-purple-300' : 'text-zinc-200'}`}>
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      ...
                      <span className="text-[11px] text-zinc-500 font-mono">{user.githubName || user.email?.split('@')[0] || "Unknown"}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-4 md:mt-0 flex items-center gap-6 self-end md:self-auto w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 border-white/5 pt-4 md:pt-0">
                  {/* 🚀 审核操作区：只有管理员且对方是 PENDING 时显示 [cite: 24, 43] */}
                  {isManager && isPending ? (
                    ...
                    <>
                      {!isPending && (
                        user.feishuLink ? (
                          <a href={user.feishuLink} target="_blank" rel="noopener noreferrer" className="group/fs flex items-center gap-2 bg-teal-500/10 border border-teal-500/30 px-3 py-1.5 rounded-lg shrink-0 hover:bg-teal-500/20 transition-all cursor-pointer">
                            <div className="w-2 h-2 rounded-full bg-teal-400"></div>
                            <span className="text-[10px] text-teal-300 font-mono uppercase tracking-wider">飞书学习链接就绪 点击查看</span>
                          </a>
                        ) : (
                          <div className="flex items-center gap-2 bg-red-500/10 border border-red-500/20 px-3 py-1.5 rounded-lg shrink-0">
                            <div className="w-2 h-2 rounded-full bg-red-500 animate-ping"></div>
                            <span className="text-[10px] text-red-400 font-mono uppercase tracking-wider">缺失飞书学习链接 点击添加</span>
                          </div>
                        )
                      )}
                      <div className="flex flex-col items-end">
                        <span className="text-[10px] uppercase font-mono tracking-widest text-zinc-500 mb-1">Status</span>
                        <span className={`text-xs font-bold tracking-widest uppercase ${isOwner ? 'text-yellow-500' : isAdmin ? 'text-purple-400' : 'text-emerald-400'}`}>{user.role}</span>
                      </div>
                    </>
                  )}
                </div>
