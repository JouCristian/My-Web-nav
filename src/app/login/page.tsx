...
import FloatingLines from "@/components/FloatingLines" // 🚀 引入 3D 悬浮线条组件
...
export default async function LoginPage({ searchParams }: { searchParams: { error?: string } }) {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-transparent p-6 relative overflow-hidden">
      ...
      {/* 🚀 替换为 3D Floating Lines 全屏背景 */}
      <div className="absolute inset-0 z-0 pointer-events-auto">
        <FloatingLines
          linesGradient={['#020205', '#1e3a8a', '#3b82f6', '#8b5cf6']} // 🚀 星际舰队的幽蓝与紫光渐变
          enabledWaves={['top', 'middle', 'bottom']}
          lineCount={[15, 25, 30]} // 更密集的线条营造深邃感
          lineDistance={[6, 5, 4]}
          bendRadius={8.0}         // 更广的鼠标弯曲感应区
          bendStrength={-1.5}      // 更强烈的物理排斥回弹
          interactive={true}
          parallax={true}
          animationSpeed={1.2}     // 适当增加流动速度
        />
      </div>
      ...
      {/* 🚀 身份校验容器 */}
      <div className="relative w-full max-w-sm animate-spring-in z-10 pointer-events-none">
        <div className="animate-breathe-glow relative w-full bg-[#0a0a0c]/70 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-10 flex flex-col items-center shadow-2xl overflow-hidden pointer-events-auto">
