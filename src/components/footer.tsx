    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className="absolute -top-9 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-lg border border-white/10 bg-black/90 backdrop-blur-md px-2.5 py-1 text-xs text-white/90 shadow-lg"
          role="tooltip"
        >
          {children}
        </motion.div>
...
function DockIcon({ children }: { children: React.ReactNode }) {
  return <div className="w-[50%] h-[50%]">{children}</div>
}
...
function DockItem({
  ...
  return (
    <motion.a
      ...
      className={`relative inline-flex items-center justify-center rounded-xl bg-white/5 border border-white/10 text-zinc-400 cursor-pointer outline-none transition-colors duration-200 hover:text-white hover:bg-white/10 hover:border-white/20 ${className}`}
      tabIndex={0}
      role="button"
    >
    ...
    <footer className="relative w-full mt-20 border-t border-white/5">
      ...
      <div className="relative max-w-5xl mx-auto px-6 py-12">
        <div className="flex flex-col items-center gap-10">
          ...
          {/* Dock 风格社交图标栏 - 使用 GlassSurface */}
          <div className="flex items-end justify-center" style={{ height: magnification + 24 }}>
            <GlassSurface
              width="auto"
              height={panelHeight}
              borderRadius={20}
            ...
            >
              <div
                ...
                className="flex items-end gap-2 px-3 h-full pb-3"
                role="toolbar"
          ...
          <div className="flex flex-col items-center gap-2 text-center">
            <p className="text-zinc-500 text-sm">
              ...
              <span className="font-[family-name:var(--font-space)] tracking-wider text-zinc-500">XIKE STARFLEET</span>
            </p>
