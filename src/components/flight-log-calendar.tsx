...
export function FlightLogCalendar({ userRole }: { userRole: string }) {
  ...
  useEffect(() => { 
    ...
    if (savedLogs) {
      try {
        ...
        console.error("读取星际档案失败:", e);
      }
    }
  }, [])

  // 🚀 核心修复 3：当 logs 变动时，自动刻录到本地磁盘
  useEffect(() => {
    if (mounted) {
      localStorage.setItem("STARFLEET_FLIGHT_LOGS", JSON.stringify(logs));
    }
  }, [logs, mounted])
  ...
  const handleSaveLog = () => {
    ...
    setLogs(prev => ({
      ...prev,
      [dateKey]: { title: editTitle, time: editTime, content: editContent }
    }))
    ...
    <AnimatePresence>
      {isModalOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.8, y: 40 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20, filter: "blur(10px)" }} transition={springConfig} className="relative w-full max-w-2xl z-10" >
            <div className="animate-modal-breathe w-full rounded-[3.5rem] bg-[#060813]/95 border p-8 md:p-12 overflow-hidden relative transition-all duration-500">
              ...
              {modalMode === "EDIT" && (
                <form ref={formRef} className="space-y-6 relative z-10" onSubmit={(e) => { e.preventDefault(); handleSaveLog(); }}>
                  <motion.div initial={false} animate={focusStyle} transition={springConfig} className="absolute z-0 rounded-2xl border-2 border-emerald-400 shadow-[0_0_25px_rgba(16,185,129,0.6)] pointer-events-none" />
                  <div className="flex gap-4">
                    <div className="flex-1 space-y-2 relative z-10">
                      <label className="text-[10px] text-emerald-500/60 uppercase tracking-[0.2em] ml-2">日志标题 / Title</label>
                      <input required type="text" value={editTitle} onChange={(e) => setEditTitle(e.target.value)} onFocus={handleFocus} onBlur={handleBlur} placeholder="输入巡航记录标题..." className="w-full bg-black/40 border border-emerald-500/20 rounded-2xl px-5 py-4 outline-none text-white font-[family-name:var(--font-space)] shadow-[inset_0_0_20px_rgba(0,0,0,0.5)] transition-colors" />
                    </div>
                    <div className="w-1/3 space-y-2 relative z-30">
                      ...
                      <div className="relative w-full">
                        ...
                        <AnimatePresence>
                          {isTimePickerOpen && (
                            <>
                              <div className="fixed inset-0 z-[40] cursor-default" onClick={(e) => { e.stopPropagation(); setIsTimePickerOpen(false); handleBlur(); }} />
                              <motion.div initial={{ opacity: 0, scale: 0.9, y: -10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: -10 }} transition={springConfig} className="absolute top-[110%] right-0 mt-2 z-[50] w-56" >
                  ...
                  <div className="space-y-2 relative z-10">
                    ...
                    <textarea required rows={6} value={editContent} onChange={(e) => setEditContent(e.target.value)} onFocus={handleFocus} onBlur={handleBlur} placeholder="支持 Markdown 语法，详细记录本次舰队巡航或集结情况..." className="ios-scrollbar w-full bg-black/40 border border-emerald-500/20 rounded-2xl px-5 py-4 outline-none text-zinc-300 resize-none shadow-[inset_0_0_20px_rgba(0,0,0,0.5)] transition-colors" />
                  </div>
                  ...
                  <div className="flex gap-4 pt-4 border-t border-emerald-500/10 relative z-20">
                    ...
                    {activeLog && <button type="button" onClick={handleClearLog} className="flex-1 py-4 rounded-2xl bg-red-500/10 border border-red-500/30 text-red-400 font-bold tracking-widest text-[10px] hover:bg-red-500 hover:text-white transition-all shadow-[0_0_20px_rgba(239,68,68,0.2)] active:scale-95 flex items-center justify-center gap-2"> <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg> 清除档案 </button>}
                    <button type="submit" className="flex-1 py-4 rounded-2xl bg-emerald-600/20 border border-emerald-500/50 text-emerald-400 font-bold tracking-widest text-[10px] hover:bg-emerald-500 hover:text-white transition-all shadow-[0_0_20px_rgba(16,185,129,0.2)] active:scale-95 flex items-center justify-center gap-2"> <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" /></svg> 刻录进档案 </button>
                  </div>
                </form>
              )}
              ...
              {modalMode === "VIEW" && (
                <div className="relative z-10 flex flex-col">
                  {activeLog ? (
                    <>
                      ...
                      <div className="bg-black/40 border border-emerald-500/20 rounded-[2rem] p-6 md:p-8 shadow-[inset_0_0_40px_rgba(0,0,0,0.5)] max-h-[40vh] overflow-y-auto emerald-scrollbar">
                        <ReactMarkdown className="text-zinc-300 text-sm md:text-base leading-relaxed break-words" components={{ h1: ({node, ...props}) => <h1 className="text-2xl font-bold text-emerald-400 mb-4 pb-2 border-b border-emerald-500/20" {...props} />, h2: ({node, ...props}) => <h2 className="text-xl font-bold text-emerald-400/90 mt-6 mb-3" {...props} />, p: ({node, ...props}) => <p className="mb-4 last:mb-0 leading-relaxed" {...props} />, strong: ({node, ...props}) => <strong className="text-emerald-300 font-bold" {...props} />, blockquote: ({node, ...props}) => <blockquote className="border-l-4 border-emerald-500/50 pl-4 py-2 mb-4 bg-emerald-500/5 rounded-r-lg text-emerald-200/80 italic" {...props} />, code: ({node, ...props}) => <code className="bg-emerald-500/10 text-emerald-300 px-1.5 py-0.5 rounded font-mono text-sm" {...props} /> }} >
                          {activeLog.content}
                        </ReactMarkdown>
                      </div>
                    </>
                  ...
                  <div className="flex gap-4 pt-8 mt-6 border-t border-emerald-500/10">
                    ...
                    {isManager && activeLog && <button type="button" onClick={handleEditExistingLog} className="flex-1 py-4 rounded-2xl bg-emerald-600/20 border border-emerald-500/50 text-emerald-400 font-bold tracking-widest text-[10px] hover:bg-emerald-500 hover:text-white transition-all shadow-[0_0_20px_rgba(16,185,129,0.2)] active:scale-95 flex items-center justify-center gap-2"> <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg> 覆写卷宗 </button>}
                  </div>
                </div>
              )}
            </div>
      ...
      <div className="relative h-full flex flex-col group/log">
        <div className="flex items-center justify-between mb-8 px-2 relative z-10">
          <div className="flex flex-col">
            ...
            <h2 className="text-2xl lg:text-3xl font-bold tracking-[0.15em] font-[family-name:var(--font-space)] text-white drop-shadow-[0_0_15px_rgba(16,185,129,0.3)]"> 航行日志 </h2>
          </div>
        ...
        <div className="flex-1 flex flex-col bg-[#02040a]/40 border border-white/5 rounded-[2rem] p-6 shadow-[inset_0_0_50px_rgba(0,0,0,0.6)] relative z-10">
          <div className="grid grid-cols-7 gap-2 mb-4 border-b border-white/5 pb-4">
            {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => <div key={d} className="text-center text-[9px] font-mono text-zinc-500 uppercase tracking-widest">{d}</div>)}
          </div>
          <div className="grid grid-cols-7 gap-2 lg:gap-3 flex-1">
            {blanks.map(i => <div key={`b-${i}`} className="aspect-square" />)}
            {days.map(day => {
              const isToday = day === new Date().getDate() && month === new Date().getMonth() && year === new Date().getFullYear();
              // 🚀 核心修复：基于唯一坐标 Key 判定是否有日志
              const hasLog = !!logs[getDateKey(day)];
              return (
                <motion.div key={day} onClick={() => handleDayClick(day)} whileHover={{ scale: 1.2, zIndex: 20 }} transition={springConfig} className={`cursor-pointer aspect-square rounded-2xl flex flex-col items-center justify-center relative transition-colors duration-300 ${isToday ? "bg-emerald-500/20 border border-emerald-500/50 shadow-[0_0_20px_rgba(16,185,129,0.3)] text-white" : "bg-white/[0.02] border border-white/5 text-zinc-400 hover:bg-emerald-500/10 hover:border-emerald-500/30 hover:text-emerald-300"}`} >
                  ...
                  {hasLog && <div className="absolute bottom-2 flex gap-0.5 z-10"> <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-[0_0_8px_#10b981]"></div> </div>}
                </motion.div>
              )
            })}
          </div>
