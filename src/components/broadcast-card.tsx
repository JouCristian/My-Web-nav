...
export function BroadcastCard({ announcement, isManager }: { announcement: any, isManager: boolean }) {
  ...
  // 🚀 简化状态：只用 isOpen，进出场全部交给 AnimatePresence
  ...
  const openReadModal = () => setIsReadOpen(true)
  const closeReadModal = () => setIsReadOpen(false)
  ...
  const executeDelete = async () => {
    if (isPending) return
    setIsPending(true)
    // 🚀 关闭删除弹窗，等待 framer-motion exit 动画完成 (~400ms 覆盖 spring 退场)
    setIsDelOpen(false)
  ...
  // 📜 公告查看弹窗 —— 与航行日志一致的 framer-motion 进出场
      ...
      {isReadOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            transition={uiSpring}
            className="relative w-full max-w-2xl z-10 my-auto max-h-[90vh] flex flex-col"
          >
  ...
  // 🗑️ 删除确认弹窗 —— 同样使用 framer-motion 一致动画
      ...
      {isDelOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="absolute inset-0 bg-[#02040a]/60 backdrop-blur-[15px]"
            onClick={() => !isPending && closeDelModal()}
          />
          <motion.div
            initial={{ opacity: 0, scale: 0.8, y: 40 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
          ...
          >
            <div className="w-full rounded-[2.5rem] border border-red-500/30 bg-[#060813]/95 p-10 shadow-2xl text-center overflow-hidden">
              ...
              <div className="flex w-full gap-4 relative z-10">
                <button onClick={closeDelModal} disabled={isPending} className="flex-1 py-4 rounded-2xl bg-white/5 border border-white/5 text-zinc-500 font-bold text-[10px] tracking-widest">取消</button>
