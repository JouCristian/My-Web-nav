...
export function BroadcastCard({ announcement, isManager }: { announcement: any, isManager: boolean }) {
  ...
  // 🚀 阵营色彩光谱
  const typeStyles: Record<string, { shadow: string, glow: string, border: string, text: string, bg: string, borderHover: string, label: string }> = {
  ...
  const readModalContent = isReadOpen ? (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      <div className={`relative w-full max-w-2xl z-10 ${isReadClosing ? "quantum-particle-out" : isReadAnimating ? "animate-slide-up-elastic" : "opacity-0"}`}>
        ...
        {/* 🚀 动态呼吸灯核心容器：将阵营色彩注入 CSS 变量 */}
        <div 
          className="quantum-breathe-dynamic w-full rounded-[3.5rem] bg-[#060813]/95 p-8 md:p-12 flex flex-col relative overflow-hidden"
          style={{ '--modal-glow': style.glow, '--modal-shadow': style.shadow, '--modal-border': style.border } as React.CSSProperties}
        >
          ...
          <div className="flex items-center justify-between border-b border-white/10 pb-6 mb-8 relative z-10">
            <div className="flex items-center gap-4">
              <div className="w-3 h-3 rounded-full animate-pulse" style={{ backgroundColor: style.shadow, boxShadow: `0 0 15px ${style.shadow}` }}></div>
              <span className={`text-sm font-mono font-bold tracking-[0.3em] uppercase ${style.text}`}>{style.label} Broadcast</span>
            </div>
            <span className="text-xs font-mono text-zinc-500 tracking-widest">{new Date(announcement.createdAt).toLocaleString()}</span>
          </div>

          <h2 className="text-2xl md:text-4xl font-bold text-white tracking-[0.1em] font-[family-name:var(--font-space)] mb-8 leading-tight relative z-10">
            {announcement.title}
          </h2>
          ...
          <div className="relative z-10 bg-black/40 border border-white/5 rounded-[2rem] p-6 md:p-8 shadow-[inset_0_0_30px_rgba(0,0,0,0.5)]">
            <div className="max-h-[40vh] overflow-y-auto ios-scrollbar pr-4 text-zinc-300 text-sm md:text-base leading-relaxed md:leading-loose font-light whitespace-pre-wrap tracking-wide">
          ...
          <div className="flex justify-between items-center mt-10 relative z-10">
            <div className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">
              Authorized by <span className="text-white font-bold">{announcement.author?.realName || "Fleet Command"}</span>
            </div>
            <button onClick={closeReadModal} className="px-8 py-4 rounded-2xl bg-white/5 border border-white/10 text-white font-bold tracking-[0.2em] uppercase text-[10px] hover:bg-white/10 transition-all active:scale-95 shadow-[0_0_20px_rgba(255,255,255,0.02)] hover:shadow-[0_0_30px_rgba(255,255,255,0.05)]">
              关闭加密档案
            </button>
          </div>
        </div>
  ...
  const delModalContent = isDelOpen ? (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      <div className={`absolute inset-0 bg-[#02040a]/60 backdrop-blur-[10px] transition-all duration-500 ${isDelAnimating ? "opacity-100" : "opacity-0"}`} onClick={closeDelModal}></div>
      <div className={`relative w-full max-w-md z-10 ${isDelClosing ? "quantum-particle-out" : isDelAnimating ? "animate-slide-up-elastic" : "opacity-0"}`}>
        <div className="w-full rounded-[2.5rem] border border-red-500/30 bg-[#060813]/95 p-10 shadow-[0_0_80px_rgba(239,68,68,0.2)] text-center overflow-hidden">
          ...
          <p className="text-zinc-400 text-sm mb-8 leading-relaxed font-mono">
            即将从星际档案中永久抹除此广播：<br/><span className="text-white font-bold">「{announcement.title}」</span><br/>此操作不可逆转。
          </p>
          <div className="flex w-full gap-4 relative z-10">
            <button onClick={closeDelModal} disabled={isPending} className="flex-1 py-3.5 rounded-2xl bg-white/5 border border-white/5 text-zinc-500 font-bold tracking-widest text-xs hover:text-white transition-all">取消</button>
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        /* 🚀 新增：动态能量呼吸灯效 (基于注入的 CSS 变量) */
        @keyframes dynamic-breathe { 
          0%, 100% { 
            transform: scale(1); 
            box-shadow: 0 0 60px var(--modal-glow), inset 0 0 20px var(--modal-glow); 
            border: 1px solid rgba(255,255,255,0.1);
          } 
          50% { 
            transform: scale(1.03); 
            box-shadow: 0 0 100px var(--modal-shadow), inset 0 0 40px var(--modal-glow); 
            border: 1px solid var(--modal-border);
          } 
        }
        .quantum-breathe-dynamic {
          animation: dynamic-breathe 2.5s cubic-bezier(0.4, 0, 0.2, 1) infinite;
        }
      ...
      {/* 🚀 卡片本体：增加 cursor-pointer 和 onClick 触发阅读 */}
      <div 
      ...
      >
        ...
        <div className="flex items-center gap-8 shrink-0 relative z-10">
          ...
          {isManager && (
            <button 
              onClick={(e) => { e.stopPropagation(); openDelModal(); }} 
              className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-red-500/20 text-zinc-700 hover:text-red-500 transition-all active:scale-90"
            >
              ✕
            </button>
          )}
