...
export function VoiceAudioPlayer({ id, src, compact = false, primary = false }: VoiceAudioPlayerProps) {
  ...
  return (
    <div className={`rounded-xl border border-cyan-100/15 bg-[#07101d]/85 ${compact ? "p-2.5" : "p-3.5"}`}>
      ...
      <div className="flex items-center gap-3">
        <motion.button type="button" onClick={() => void togglePlayback()} whileTap={voiceTap} transition={voiceFastSpring} className={`${compact ? "h-9 w-9" : "h-11 w-11"} grid shrink-0 cursor-pointer place-items-center rounded-full border border-cyan-100/25 bg-cyan-200/[0.12] text-cyan-50 transition-colors hover:bg-cyan-200/[0.18] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-200/50`} aria-label={playing ? "暂停音频" : "播放音频"}>
        ...
        <div className="min-w-0 flex-1">
          <input
            ...
            aria-label="音频播放进度"
          />
          <div className="mt-1.5 flex items-center justify-between font-mono text-[10px] text-cyan-50/55"><span>{formatTime(currentTime)}</span><span>{formatTime(duration)}</span></div>
        </div>
